			...
			{list.map(item =>
				<div key={item.id} className='list-container__item'>
					<div className='list__movie-image'>
						...
						{
						item.poster_path ? (
							<Link to={`/movie/${item.id}-${titleURL(item.title)}`}><img src={`https://image.tmdb.org/t/p/w185_and_h278_bestv2${item.poster_path}`} alt={item.title}/></Link>
						)
						: 
						(
							<Link to={`/movie/${item.id}`}>
								<div className='list__movie-no_image_holder'><span>{item.title}</span></div>
						    </Link>)
						}
					</div>
